...
for learn_var, loss_gen, loss_wvar, loss_svar, do_linear_encoder in (scenarios):
    ...
    for t in tqdm(range(10)):
        ...
        if do_linear_encoder:
            post_loss_var = np.diag(dataset.pz_condx_var - model.encoder.var_encoder.detach().cpu().numpy())
        else:
        ...
        if not do_linear_encoder:
            ...
            plugin_cdf = norm.cdf(0, loc=seq_mean[:, 0], scale=np.sqrt(seq_var[0, 0]))  # In this case the encoder learns a constant full
        ...
        # k_hat
        ratios = trainer.test_set.log_ratios(n_samples_mc=100).exp().detach().numpy()
    ...
    res = {
        ...
        "AVE L1 ERROR EXACT <-> PLUGIN": (np.mean(l1_err_ex_plugin), np.std(l1_err_ex_plugin)),
