...
class SyntheticMixtureGaussianDataset(GaussianDataset):
    def __init__(self, dim_z=10, dim_x=10, nu=0.5, n_centers=3, n_samples=1000, seed=0):
        ...
        self.px_condxz_mean_link = np.dot(self.pz_condxw_var, np.dot(self.A.T, inv_gamma))
        self.px_condxz_mean_bias = np.dot(self.pz_mean, self.pz_condxw_var)  # shape (n_centers, dim_z)

        # store complete covariance matrix of x, z conditioned on w
        off_diag_block = - np.dot(self.A.T, inv_gamma)
        precision_joint = np.block([[inv_pz_condxw_var, off_diag_block], [off_diag_block.T, inv_gamma]])
